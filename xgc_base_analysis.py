...
def blob_detection(fname,timestep,initial_timestep):
    ...
    for i in range(len(image)):
        ...
        if i ==1:
            ...
            np.savez("/ssd/xgc_blob.npz", blob_number = blob_number, blob_diameter = blob_diameter, blob_area = blob_area, overlap_ratio = overlap_ratio)
            print "blob_number = ", blob_number
            print "blob_diameter = ", blob_diameter
            print "blob_area = ", blob_area
            print "overlap_ratio = ", overlap_ratio 
        plt.savefig("/ssd/xgc_30_120_"+str(timestep)+'_1_blobed.pdf', dpi=600,format='pdf')
...
deci_ratio = 8192
initial_timestep = 1320
time_interval = 60
reduced_len = 5485
finer_len = 44928785
fp = np.load("/ssd/xgc_30_120_base_plot.npz")
data = fp['finer']
data_r = fp['finer_r']
data_z = fp['finer_z']

fp = np.load("/ssd/xgc_30_120_base_psnr.npz")
finer = fp['psnr_finer']
...
start = time.time()
fname_head = "/hdd/xgc_30_120_macro_base"
plot_name = fname_head + ".png"
plot(data,data_r,data_z,plot_name) 
blob_detection(plot_name,0,0)
end = time.time()
